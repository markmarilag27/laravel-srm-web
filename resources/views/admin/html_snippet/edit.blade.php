...
{{-- title --}}
@push('title') {{ $htmlSnippet->title }} @endpush

@section('content')
<form id="{{ $htmlSnippet->uuid }}" action="{{ route('admin.htmlSnippet.update', ['htmlSnippet' => $htmlSnippet]) }}" method="POST">
    @csrf
        ...
        <div class="bg-white py-6 px-6 rounded-sm mt-4 shadow-sm">
            <div>
                ...
                <input
                    ...
                    placeholder="Title"
                    value="{{ $htmlSnippet->title }}"
                    required
                >
            </div>
            <div class="pt-6">
                <label class="block mb-2 text-sm font-medium text-gray-900">HTML Snippet</label>
                <textarea rows="4" name="description" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Your HTML snippets..." value="{{ $htmlSnippet->description }}"></textarea>
            </div>
        </div>
    </div>
</form>
